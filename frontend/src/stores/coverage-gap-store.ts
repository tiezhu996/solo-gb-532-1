...
import type { GenerateResurveyPlan, TransectPlan } from '../types/transect-plan'

interface GapStore{items:CoverageGap[];selected:CoverageGap|null;evidence:CoverageEvidence|null;loading:boolean;fetch:()=>Promise<void>;select:(item:CoverageGap)=>void;detect:(body:DetectCoverage)=>Promise<void>;transition:(item:CoverageGap,target:GapState,note:string)=>Promise<void>;resurvey:(item:CoverageGap,body:GenerateResurveyPlan)=>Promise<TransectPlan>}
export const useCoverageGapStore=create<GapStore>((set)=>({items:[],selected:null,evidence:null,loading:false,
  ...
  resurvey:async(item,body)=>{const response=await coverageGapApi.resurveyPlan(item.id,body);return response.data}
}))
